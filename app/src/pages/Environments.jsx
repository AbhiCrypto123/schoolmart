...
const Environments = () => {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center text-[#004a8e] font-black tracking-widest uppercase py-20">Loading Environments...</div>;
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-6 pt-4">
        ...
        <section className="grid grid-cols-1 md:grid-cols-12 gap-3 lg:gap-4 mb-4 lg:mb-6">
           ...
           <div className="md:col-span-12 lg:col-span-5 bg-white rounded-[40px] p-8 lg:p-14 flex flex-col justify-center border border-gray-100 shadow-sm relative overflow-hidden group min-h-[300px] lg:min-h-[400px]">
              ...
              <h1 className="text-4xl lg:text-6xl font-black font-heading leading-[0.9] mb-8 tracking-tighter text-gray-900 uppercase" dangerouslySetInnerHTML={{ __html: d.hero?.titleHtml }} />
              <p className="text-gray-400 text-[10px] lg:text-[11px] font-bold uppercase tracking-widest max-w-sm leading-relaxed">
                 {d.hero?.subtitle}
              </p>
           </div>
           ...
           <div className="md:col-span-6 lg:col-span-3 bg-[#004a8e] rounded-[40px] p-8 text-white flex flex-col justify-between shadow-lg">
              ...
              <button 
                ...
                className="w-full py-4 bg-white text-[#004a8e] font-black rounded-2xl text-[10px] uppercase tracking-widest active:scale-95 transition-all shadow-xl"
              >
                 {d.actionCard?.btnText}
              </button>
           </div>
           
           {/* SUB-BLOCKS (HIGH DENSITY) */}
           <div className="md:col-span-12 lg:col-span-12 grid grid-cols-2 lg:grid-cols-4 gap-3">
              {(d.subBlocks || []).map((sb, i) => {
                const Icon = ICONS[sb.icon] || Sparkles;
                return (
                  <div key={i} className="bg-white rounded-[25px] p-6 border border-gray-100 shadow-sm flex items-center justify-between group h-[80px]">
                     <div className="flex flex-col">
                        <span className="text-[11px] font-black text-gray-900 uppercase tracking-tighter">{sb.title}</span>
                        <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest leading-none">{sb.subtitle}</span>
                     </div>
                     <Icon className="text-gray-200 group-hover:text-[#004a8e] transition-colors" size={24} />
                  </div>
                );
              })}
              <div className="bg-blue-50/50 rounded-[25px] p-6 border border-blue-100 shadow-sm flex items-center justify-between group h-[80px]">
                 <span className="text-[9px] font-black uppercase tracking-[0.1em] text-[#004a8e] leading-tight">Spatial Research Indices</span>
                 <Layers className="text-[#004a8e]/40" size={24} />
              </div>
           </div>
        </section>
        ...
        <section className="py-2 border-t border-gray-100 mt-4">
           <div className="flex items-center justify-between py-4 mb-4">
              <h2 className="text-[14px] font-black text-gray-900 uppercase tracking-[0.2em] font-heading">Institutional Case Studies</h2>
           ...
           <div className="lg:col-span-7">
              ...
              <div className="grid grid-cols-2 gap-3">
                 {(d.infoGrid?.points || []).map((item, i) => (
                    <div key={i} className="flex items-center gap-3 text-[10px] font-black text-gray-900 uppercase tracking-widest bg-gray-50 p-5 rounded-2xl hover:border-[#004a8e]/20 border border-transparent transition-all">
