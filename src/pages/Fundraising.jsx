      ...
      <section className="bg-gray-900 py-16 px-6 relative overflow-hidden text-center text-white border-b border-white/10">
        <div className="max-w-4xl mx-auto relative z-10">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-white/5 border border-white/10 rounded-full mb-6 text-sm-blue">
            <Shield size={12} />
            <span className="text-[11px] font-black uppercase tracking-[0.4em]">Performance Advisory</span>
          </div>
          ...
          <div className="flex justify-center flex-col md:flex-row gap-4">
             <button className="px-10 py-5 bg-sm-blue text-white font-black rounded-xl text-[12px] uppercase tracking-widest hover:bg-white hover:text-gray-900 transition-all shadow-xl shadow-blue-500/10 active:scale-95 duration-500 group">
                Schedule Performance Audit <ArrowRight size={14} className="inline ml-2 group-hover:translate-x-1" />
             </button>
             <button className="px-10 py-5 border border-white/20 text-white font-black rounded-xl text-[12px] uppercase tracking-widest hover:bg-white/5 transition-all">
                Download Brochure
             </button>
          </div>
        </div>
         ...
         <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {ADVISORY_FOCUS.map((focus, i) => (
               <div key={i} className="flex flex-col p-10 bg-white rounded-[40px] border border-gray-100 shadow-xl shadow-gray-200/50 hover:shadow-2xl transition-all group">
                  ...
                  <p className="text-gray-500 text-[13px] leading-relaxed mb-10 flex-grow min-h-[4.5rem] line-clamp-3">{focus.desc}</p>
      ...
      <section className="py-16 px-6 bg-white overflow-hidden">
         <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-16">
            <div className="lg:w-1/3">
               ...
               <p className="text-gray-500 text-[13px] font-bold uppercase tracking-widest leading-loose mb-10">
                  Real numbers, real outcomes. Institutional growth facilitated by Board Expertise.
               </p>
               <button className="flex items-center gap-2 text-sm-blue font-black text-[12px] uppercase tracking-widest hover:gap-3 transition-all">
