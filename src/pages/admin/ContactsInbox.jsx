...
import { Mail, Phone, Calendar, ChevronDown, CheckCircle, RotateCcw, MapPin } from 'lucide-react';
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4">
        {filtered.map(c => (
          <div key={c.id} className={`bg-white rounded-2xl border shadow-sm p-5 flex flex-col sm:flex-row gap-5 transition-all ${c.status === 'new' ? 'border-purple-200 shadow-purple-500/5' : 'border-gray-100'}`}>
            {/* Avatar block */}
            <div className="shrink-0 flex sm:flex-col items-center sm:w-32 gap-3 text-center">
              ...
              <div className="sm:w-full min-w-0 text-left sm:text-center">
                <p className="font-bold text-sm text-gray-800 truncate" title={c.schoolName || c.name}>{c.schoolName || c.name}</p>
                <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest">{c.name}</p>
                <a href={`mailto:${c.email}`} className="text-xs text-blue-500 hover:underline truncate block mt-1" title={c.email}>{c.email}</a>
                <div className="flex flex-col gap-1 mt-2">
                  {c.phone && <a href={`tel:${c.phone}`} className="text-[10px] text-gray-500 font-bold flex items-center justify-start sm:justify-center gap-1"><Phone size={10} /> {c.phone}</a>}
                  {c.pinCode && <span className="text-[10px] text-gray-400 font-black uppercase flex items-center justify-start sm:justify-center gap-1"><MapPin size={10} /> {c.pinCode}</span>}
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col gap-3 min-w-0">
              <div className="bg-gray-50/80 rounded-2xl rounded-tl-none sm:rounded-tl-2xl border border-gray-100 p-4">
                <p className="text-sm font-bold text-gray-800 mb-1">{c.subject || 'Institutional Inquiry'}</p>
              ...
              <div className="flex items-center gap-2 justify-end">
                {c.status === 'new' && (
                  <button onClick={() => changeStatus(c.id, 'read')} className="px-4 py-2 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-xl text-xs font-bold transition-colors">
                    Mark as Read
                  </button>
                )}
                {c.status !== 'replied' && (
                  <button onClick={() => changeStatus(c.id, 'replied')} className="px-4 py-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-600 rounded-xl text-xs font-bold transition-colors flex items-center gap-1.5">
                    <CheckCircle size={14} /> Mark Replied
                  </button>
                )}
                {c.status === 'replied' && (
                  <button onClick={() => changeStatus(c.id, 'read')} className="px-4 py-2 bg-gray-50 hover:bg-gray-100 text-gray-600 rounded-xl text-xs font-bold transition-colors flex items-center gap-1.5">
