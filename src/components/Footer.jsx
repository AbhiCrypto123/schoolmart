    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-6 py-20">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-12 gap-y-16">
          ...
          <div className="flex flex-col">
            <h3 
              className="text-[12px] font-black uppercase tracking-[0.2em] mb-6 font-heading text-left"
              style={{ color: headingColor }}
            >
              Identity
            </h3>
            <div className="text-[13px] leading-relaxed font-medium text-gray-400 text-left max-w-[280px]">
              <p className="mb-8">
                <Link 
                  to="/" 
                  className="font-bold border-b transition-all duration-200"
              ...
              <div className="pt-8 border-t border-white/5">
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white/30 block mb-3">Heritage</span>
                <p className="text-[13px] text-white/50 leading-relaxed">
                  An initiative of thirdeye group. Helping <span className="text-white font-black">4000+</span> partner schools build excellence since <span className="text-white font-black">7 years</span>.
                </p>
              </div>
            </div>
          </div>
          ...
          {columns.map((col, idx) => (
            <div key={idx}>
              <h3 
                className="text-[13px] font-black uppercase tracking-[0.2em] mb-8 font-heading"
                style={{ color: headingColor }}
              >
                {col.title}
              </h3>
              <ul className="space-y-4">
                {(col.links || []).map((link) => (
                  <li key={link.name || link.label}>
                    <Link 
                      to={link.path} 
                      className="text-[12px] hover:text-white transition-colors duration-200 font-bold uppercase tracking-widest block"
        ...
        {/* Strategic Insights - Organized Grid */}
        <div className="mt-24 pt-12 border-t border-white/5">
          <div className="mb-10">
            <h3 className="text-[11px] font-black uppercase tracking-[0.4em] text-white/20 font-heading">Strategic Insights</h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-12 gap-y-6">
            {trendingLinks.map((item) => (
              <Link
                key={item}
                className="text-[11px] font-black uppercase tracking-[0.1em] text-white/40 hover:text-sm-blue transition-colors duration-200 flex items-center gap-2 group"
              >
                <div className="w-1.5 h-1.5 rounded-full bg-white/10 group-hover:bg-sm-blue transition-colors" />
                {item}
              </Link>
            ))}
          </div>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="text-[12px] font-black uppercase tracking-[0.2em] opacity-50">
            {copyright}
          </div>
          ...
          <div className="flex items-center gap-10">
            {bottomLinks.map((link, idx) => (
              <Link 
                key={idx} 
                to={link.path} 
                className="text-[12px] font-black uppercase tracking-[0.2em] opacity-50 hover:text-white transition-all duration-200"
              >
                {link.label}
              </Link>
            ))}
