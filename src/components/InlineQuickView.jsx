...
import { X, FileText, Download, ExternalLink, MessageSquare, Info, BarChart3, Clock, CheckCircle2, ArrowUpRight, ShieldCheck } from 'lucide-react';
...
const InlineQuickView = ({ isOpen, onClose, data }) => {
  ...
  const handleAction = (e, target = '/registration') => {
    e.preventDefault();
    if (!user) {
      setShowCaution(true);
      return;
    }
    // If logged in, we could potentially open a quote form here
    // For now, redirecting to registration or a specific quote hub
    navigate(target);
  };
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100 bg-gray-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[12px] font-bold text-sm-blue uppercase tracking-[0.2em] mt-1.5">{data.cat || data.c || data.subcategory || data.category || 'Resource Overview'}</p>
            </div>
          </div>
          <button 
            onClick={onClose}
          ...
          <div className="lg:w-3/5 p-8 lg:p-10 space-y-10 bg-white">
             ...
             <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                <div className="bg-blue-50/50 p-5 rounded-2xl border border-blue-100/50">
                   ...
                   <h4 className="text-[12px] font-black text-gray-400 uppercase tracking-widest mb-1">{data.stats?.[0]?.label || 'Impact Scale'}</h4>
                ...
                <div className="bg-emerald-50/50 p-5 rounded-2xl border border-emerald-100/50">
                   ...
                   <h4 className="text-[12px] font-black text-gray-400 uppercase tracking-widest mb-1">{data.stats?.[1]?.label || 'Install Time'}</h4>
                ...
                <div className="bg-purple-50/50 p-5 rounded-2xl border border-purple-100/50">
                   ...
                   <h4 className="text-[12px] font-black text-gray-400 uppercase tracking-widest mb-1">{data.stats?.[2]?.label || 'Standard'}</h4>
             ...
             <div className="bg-gray-50 rounded-[25px] p-8 text-gray-900 border border-gray-100 relative overflow-hidden group">
                ...
                <h3 className="text-sm font-black uppercase tracking-[0.25em] mb-6 text-sm-blue flex items-center gap-2">
                    ...
                    {(data.resources?.length > 0 && data.resources.some(r => r.name)) ? data.resources.filter(r => r.name).map((feat, idx) => {
                      ...
                      return (
                        <Component 
                          key={idx} 
                          {...extraProps}
                          className={`flex items-center justify-between p-3.5 bg-white border border-gray-200 rounded-xl transition-all ${isClickable ? 'hover:bg-gray-50 hover:border-sm-blue cursor-pointer group/feat' : ''}`}
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                               <p className="text-[13px] font-bold text-gray-900 uppercase tracking-tight">{feat.name}</p>
                               {feat.size && <p className="text-[11px] text-gray-400 uppercase font-black">{feat.size}</p>}
                            </div>
                          </div>
                          {isClickable && <ExternalLink size={12} className="text-gray-300 group-hover/feat:text-sm-blue transition-all" />}
                        </Component>
                      );
                    }) : [
                      ...
                      <div key={idx} className="flex items-center gap-4 p-3.5 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all group/feat">
                         ...
                         <div>
                            <p className="text-[13px] font-bold text-gray-900 uppercase tracking-tight">{feat.name}</p>
                            <p className="text-[11px] text-gray-400 uppercase font-black">{feat.size}</p>
                         </div>
                      </div>
                    ))}
                </div>
                ...
                <div className="mt-8 flex gap-3">
                  <button 
                    onClick={(e) => handleAction(e, ctaLink)}
                  ...
                  <a 
                    ...
                    className="px-6 py-3 bg-white border border-gray-200 text-gray-900 font-black rounded-xl text-[12px] uppercase tracking-widest hover:bg-gray-50 transition-all flex items-center gap-2"
                  >
      ...
      {showCaution && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-in fade-in transition-all">
           <div className="bg-white rounded-[40px] p-10 lg:p-16 max-w-xl w-full shadow-3xl text-center relative overflow-hidden border border-gray-100">
              ...
              <div className="w-20 h-20 bg-blue-50 text-sm-blue rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-sm">
                 <ShieldCheck size={40} strokeWidth={1.5} />
              </div>
              
              <h2 className="text-3xl lg:text-5xl font-black text-gray-900 font-heading mb-6 leading-tight uppercase tracking-tighter">Portal <br/> <span className="text-sm-blue">Verification.</span></h2>
              <p className="text-gray-400 text-[12px] lg:text-[13px] font-bold uppercase tracking-[0.2em] mb-12 leading-loose mx-auto max-w-sm">
                 Quotation indices and technical specifications are protected for verified institutional members. Please login to your portal to proceed.
              </p>
              
              <div className="space-y-4">
                 <button 
                   onClick={() => navigate('/login')}
                   className="w-full py-5 bg-sm-blue text-white font-black rounded-full uppercase tracking-widest text-[13px] shadow-2xl hover:bg-gray-900 transition-all flex items-center justify-center gap-3 active:scale-95"
                 >
